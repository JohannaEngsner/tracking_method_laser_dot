...
while(vidObj.hasFrame)
    ...
    seSize = 2; % Choose this depending on how far away from the wall the patient is sitting. (If 1m away, choose seSize = 1). 
    % Original: seSize = 2.
    ...
    % Choosing the radius of the laser dot
    radius = 5;
    if seSize == 1
        radius = 3;
    end
    ...
    imshow(binaryFrame)
    hold on
    viscircles(centers,radius,'Color', '#e697c6');
    waitbar(a / n, h);
...
message = [message, sprintf('Green zone: %.2f %%\n', results(2)*100)];
...
if binaryThinLine(int64(allCenters(i,2)) + offset_y, int64(allCenters(i,1)) + offset_x) == 1
...
if onThin == 0    
if binaryThickLine(int64(allCenters(i,2)),int64(allCenters(i,1))) == 1
    thickFrames=thickFrames+1; % Updating counter if the coordinates match the zone

elseif binaryBlueArea(int64(allCenters(i,2)),int64(allCenters(i,1))) == 1
    blueFrames=blueFrames+1; % Updating counter if the coordinates match the zone

elseif binaryYellowArea(int64(allCenters(i,2)),int64(allCenters(i,1))) == 1
    yellowFrames=yellowFrames+1; % Updating counter if the coordinates match the zone

elseif binaryRedArea(int64(allCenters(i,2)),int64(allCenters(i,1))) == 1
...
binaryIm = 1-imbinarize(grayScaleImage, 0.9); % Turning it into a binary image
end